...
__global__ void grid_mul(int npol, int gridsize, int nbatch, int nchan,
...
//      int tid_xy=tid_x*blk_y+tid_y;
	...
	int bid2 = ((bid_x * grid_y + bid_y) * npol * grid_z  + bid_z) * blk_x ;
        //int bid  = (bid_x * tt  * grid_z  + bid_z) * blk_x ;
        //int bid2 = (bid_x * grid_y * grid_z  + bid_z) * blk_x ;
...
template<typename In, typename Out>
inline void launch_grid_mul_kernel(int npol, bool polmajor, int gridsize, int nbatch, int nchan, 
                               In*  d_in,
                               Out* d_out,
    ...
    //dim3 grid(nbatch*nchan,npol,grid_z);
 //cout << endl << " batch " << nbatch << " polz " << npol << " bool " << polmajor << endl ;    
  ...
  // cout << "  Block size is " << block.x << " by " << block.y << " by " << block.z << endl;
    ...
    BF_CHECK_CUDA_EXCEPTION(cudaLaunchKernel((void*)grid_mul<In,Out>,
						 grid, block,
...
public:
    BFgmul_impl() : _polmajor(true), _stream(g_cuda_stream) {}
...
#define LAUNCH_GMUL_KERNEL(IterType,OterType) \
        launch_grid_mul_kernel(npol, _polmajor, _gridsize, nbatch, nchan, \
        ...
        switch( in->dtype ) {
            case BF_DTYPE_CI4:
                if( in->big_endian ) {
                    switch( out->dtype ) {
                        case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(nibble2*, Complex32*);  break;
                        case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(nibble2*, Complex64*);  break;
                        default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                    };
                } else {
                    switch( out->dtype ) {
                        case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(blenib2*, Complex32*);  break;
                        case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(blenib2*, Complex64*);  break;
                        default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                    };
                }
                break;
            case BF_DTYPE_CI8:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(char2*, Complex32*);  break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(char2*, Complex64*);  break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                };
                break;
            case BF_DTYPE_CI16:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(short2*, Complex32*); break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(short2*, Complex64*); break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                }
                break;
            case BF_DTYPE_CI32:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(int2*, Complex32*); break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(int2*, Complex64*); break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                }
                break;
            case BF_DTYPE_CI64:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(long2*, Complex32*); break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(long2*, Complex64*); break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                }
                break;
            case BF_DTYPE_CF32:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(float2*, Complex32*);   break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(float2*, Complex64*);   break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                }
                break;
            case BF_DTYPE_CF64:
                switch( out->dtype ) {
                    case BF_DTYPE_CF32: LAUNCH_GMUL_KERNEL(double2*, Complex32*);  break;
                    case BF_DTYPE_CF64: LAUNCH_GMUL_KERNEL(double2*, Complex64*);  break;
                    default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
                }
                break;
            default: BF_ASSERT_EXCEPTION(false, BF_STATUS_UNSUPPORTED_DTYPE);
        }
#undef LAUNCH_GMUL_KERNEL
...
BFstatus bfgMulCreate(BFgmul* plan_ptr) {
    BF_TRACE();
    BF_ASSERT(plan_ptr, BF_STATUS_INVALID_POINTER);
    BF_TRY_RETURN_ELSE(*plan_ptr = new BFgmul_impl(),
                       *plan_ptr = 0);
}
